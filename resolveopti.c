...
/*   Updated: 2019/02/22 19:05:57 by czhang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	*max_x_before_current(int ***coord, int current, int len)
{
	...
	if (!(max = (int*)malloc(sizeof(int) * len)))
		return (0);
	i_tetri = -1;
	while (++i_tetri < current)
	{
		i_block = -1;
		while (++i_block < 4)
		{
			line = -1;
			while (++line < len)
				if (coord[i_tetri][i_block][1] > max[line])
					max[line] = coord[i_tetri][i_block][1];
		}
	}
	return (max);
}
